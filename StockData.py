...
class StockData:
	...
	#Koscom Data Mall에서 주식고유코드, 주식이름이 적힌 html문서를 가져옴.
	...
	def parseCodeHTML(self, html, market_type):
		...
		for a_option in options:
			#이상한 값 들어왔으면 무시
			if len(a_option.text)<=8:
				continue #잘못 된 값은 무시
			text =a_option.text
			i_code1 = text.find('(')
			i_code2 = text.find(')')
			code = a_option.text[i_code1+1:i_code2]
